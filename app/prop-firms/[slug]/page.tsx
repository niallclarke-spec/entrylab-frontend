...
import { propFirmsTable, articlesTable, comparisonsTable, reviewsTable } from "@/lib/schema";
import { eq, and, or, desc } from "drizzle-orm";
import { notFound } from "next/navigation";
import Link from "next/link";
import { Star, TrendingUp, ExternalLink, CheckCircle2, XCircle, Award } from "lucide-react";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = firm.seoTitle || `${stripHtml(firm.name)} Review ${currentYear()} | EntryLab`;
  const description = firm.seoDescription || firm.tagline ||
    `Comprehensive review of ${stripHtml(firm.name)}. Profit splits, evaluation fees, funding, and more.`;

  return {
    title,
    description,
    openGraph: { title, description, url: `${SITE_URL}/prop-firms/${slug}`, type: "website" },
    twitter: { title, description },
    alternates: { canonical: `${SITE_URL}/prop-firms/${slug}` },
  };
}
...
export default async function PropFirmReviewPage({ params }: Props) {
  ...
  const [relatedArticles, relatedComparisons, reviews] = await Promise.all([
    db.select({ title: articlesTable.title, slug: articlesTable.slug })
      .from(articlesTable)
      .where(and(eq(articlesTable.status, "published"), eq(articlesTable.relatedPropFirm, slug)))
      .limit(6),
    db.select({ slug: comparisonsTable.slug, entityAName: comparisonsTable.entityAName, entityBName: comparisonsTable.entityBName })
      ...
      .where(and(eq(comparisonsTable.entityType, "prop_firm"), eq(comparisonsTable.status, "published"),
        or(eq(comparisonsTable.entityASlug, slug), eq(comparisonsTable.entityBSlug, slug))))
      .limit(6),
    db.select().from(reviewsTable)
      .where(and(eq(reviewsTable.firmSlug, slug), eq(reviewsTable.firmType, "prop_firm"), eq(reviewsTable.status, "approved")))
      .orderBy(desc(reviewsTable.createdAt)).limit(10),
  ]);

  const name = stripHtml(firm.name);
  const rating = Number(firm.rating || 0);
  const pros = firm.pros || [];
  const cons = firm.cons || [];
  const highlights = firm.highlights || [];

  const structuredData = {
    "@context": "https://schema.org",
    "@type": "FinancialService",
    "@id": `${SITE_URL}/prop-firms/${slug}#organization`,
    name,
    description: firm.seoDescription || firm.tagline || `${name} prop firm review`,
    url: firm.affiliateLink || `${SITE_URL}/prop-firms/${slug}`,
    ...(firm.headquarters && { address: { "@type": "PostalAddress", addressLocality: firm.headquarters } }),
    ...(firm.yearFounded && { foundingDate: firm.yearFounded }),
    aggregateRating: {
      "@type": "AggregateRating",
      ratingValue: rating, bestRating: 5, worstRating: 1,
      ratingCount: Math.max(reviews.length, 1), reviewCount: Math.max(reviews.length, 1),
    },
  };
  ...
  const breadcrumbData = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name, item: `${SITE_URL}/prop-firms/${slug}` },
    ],
  };

  const faqItems: { q: string; a: string }[] = [];
  if (firm.profitSplit) faqItems.push({ q: `What is ${name}'s profit split?`, a: `${name} offers a ${firm.profitSplit} profit split.` });
  if (firm.evaluationFee) faqItems.push({ q: `How much does the ${name} evaluation cost?`, a: `The ${name} evaluation starts at ${firm.evaluationFee}.` });
  if (firm.maxFundingSize) faqItems.push({ q: `What is the maximum funding from ${name}?`, a: `${name} offers funding up to ${firm.maxFundingSize}.` });
  if (firm.maxDrawdown) faqItems.push({ q: `What is ${name}'s maximum drawdown?`, a: `${name} allows a maximum drawdown of ${firm.maxDrawdown}.` });
      ...
      <section style={{ background: "#1a1e1c" }} className="px-4 sm:px-6 py-10">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-start gap-6">
            {firm.logoUrl && (
              <img src={firm.logoUrl} alt={`${name} logo`} className="w-16 h-16 rounded-xl object-contain bg-white p-2" />
            )}
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">{name} Review {currentYear()}</h1>
              <p className="text-[#adb2b1] max-w-xl">{firm.tagline}</p>
              <div className="flex items-center gap-4 mt-4">
                <div className="flex items-center gap-1.5 text-lg font-bold" style={{ color: "#2bb32a" }}>
                  <Star className="h-5 w-5 fill-current" /> {rating.toFixed(1)}/5
                </div>
                {firm.isVerified && (
      ...
      <section style={{ background: "#f8faf8" }} className="px-4 sm:px-6 py-10">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-[1fr_350px] gap-8">
          <div className="min-w-0">
            ...
            <div className="rounded-xl p-6 mb-6" style={{ background: "#fff", border: "1px solid #e8edea" }}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {[
                  ...
                  { label: "Max Funding", value: firm.maxFundingSize },
                  { label: "Evaluation Fee", value: firm.evaluationFee },
                  { label: "Challenge Types", value: firm.challengeTypes },
                  ...
                  { label: "Platforms", value: firm.platformsList?.join(", ") || undefined },
                  { label: "Founded", value: firm.yearFounded },
                    ...
                    <ul className="space-y-2">
                      {pros.map((pro, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm" style={{ color: "#374151" }}>
                          <CheckCircle2 className="h-4 w-4 mt-0.5 flex-shrink-0" style={{ color: "#16a34a" }} /> {pro}
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                {cons.length > 0 && (
                  <div className="rounded-xl p-5" style={{ background: "#fff", border: "1px solid #e8edea" }}>
                    ...
                    <ul className="space-y-2">
                      {cons.map((con, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm" style={{ color: "#374151" }}>
                          <XCircle className="h-4 w-4 mt-0.5 flex-shrink-0" style={{ color: "#dc2626" }} /> {con}
                        </li>
                      ))}
                    </ul>
                  </div>
            ...
            {/* Review content */}
            {firm.content && (
              <div className="rounded-xl p-6 md:p-8 mb-6 prose max-w-none" style={{ background: "#fff", border: "1px solid #e8edea" }}
                dangerouslySetInnerHTML={{ __html: firm.content }} />
            )}
              ...
              <div className="rounded-xl p-6 mb-6" style={{ background: "#fff", border: "1px solid #e8edea" }}>
                ...
                <ul className="space-y-2">
                  {relatedArticles.map((a) => (
                    <li key={a.slug}><Link href={`/prop-firms/${slug}/${a.slug}`} className="text-sm hover:underline" style={{ color: "#2bb32a" }}>{a.title}</Link></li>
                  ))}
                </ul>
              </div>
            )}
              ...
              <div className="rounded-xl p-6" style={{ background: "#fff", border: "1px solid #e8edea" }}>
                ...
                <ul className="space-y-2">
                  {relatedComparisons.map((c) => (
                    <li key={c.slug}><Link href={`/compare/${c.slug}`} className="text-sm hover:underline" style={{ color: "#2bb32a" }}>{c.entityAName} vs {c.entityBName}</Link></li>
                  ))}
                </ul>
              </div>
            )}
          ...
          <aside className="space-y-6">
            {firm.affiliateLink && (
              <div className="rounded-xl p-6 sticky top-24" style={{ background: "#fff", border: "1px solid #e8edea" }}>
                <h3 className="font-semibold mb-3" style={{ color: "#111827" }}>Start Challenge</h3>
                {firm.discountCode && (
                  <p className="text-sm mb-3 p-2 rounded-lg" style={{ background: "rgba(43,179,42,0.05)", color: "#16a34a" }}>
                    Use code <strong>{firm.discountCode}</strong> {firm.discountAmount && `for ${firm.discountAmount} off`}
                  </p>
                )}
                <a href={firm.affiliateLink} target="_blank" rel="noopener noreferrer sponsored"
            ...
            {highlights.length > 0 && (
              <div className="rounded-xl p-6" style={{ background: "#fff", border: "1px solid #e8edea" }}>
                ...
                <ul className="space-y-2">
                  {highlights.map((h, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm" style={{ color: "#374151" }}>
                      <Award className="h-4 w-4 mt-0.5 flex-shrink-0" style={{ color: "#2bb32a" }} /> {h}
                    </li>
                  ))}
                </ul>
              </div>
